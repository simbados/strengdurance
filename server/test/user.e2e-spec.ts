  ...
  describe('Validation error tests', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect(response => {
          response.body.error.includes(
            'property username has failed the following constraints: isNotEmpty',
          );
          response.body.message ===
            'Validation failed, input parameters are not valid';
        });
    });
    ...
    it('/register (POST) without password', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect(response => {
          response.body.error.includes(
            'property password has failed the following constraints: isNotEmpty',
          );
          response.body.message ===
            'Validation failed, input parameters are not valid';
        });
